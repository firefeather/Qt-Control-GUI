	...
	//Delete all
        ...
        for (int i=0; i<lines; i++) {
			...
			int logicalIndex=tableView->horizontalHeader()->logicalIndex(j);
                        QString txt = model->data(model->index((pagenum-1)*lines+i,logicalIndex)).toString();
                        QGraphicsTextItem *item=new QGraphicsTextItem();
                        item->setFont(font);
                        txt=fmt->elidedText(txt,Qt::ElideRight,QVariant(colSizes[logicalIndex].rawValue()).toInt()-8);
                        item->setPlainText(txt);
			item->moveBy((csize*columnMultiplier)+leftMargin,(i*rowHeight)+topMargin+headerSize+(rowHeight*2));
                        csize=csize+QVariant(colSizes[logicalIndex].rawValue()).toInt();
                        pageScene.addItem(item);
