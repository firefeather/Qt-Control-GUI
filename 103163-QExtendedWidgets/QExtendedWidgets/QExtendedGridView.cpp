...
// *****************************************************************************
class QBandsProxyModel : public QProxyModel
{
	public :
		...
		QBandsProxyModel ( QHdrModelType type, QObject * parent = 0 ) 
			: QProxyModel ( parent ), m_Type ( type ) {};
		...
		QVariant headerData ( int section, Qt::Orientation orientation, int role ) const
		{
			...
			// Footer makes a "Bypass", changing section sign ( 0 => -1, 1 => -2 )
	 		if ( m_Type == hmtFooter )
	 			return model()->headerData ( -section - 1, Qt::Horizontal, role );
	 			
			if ( role != Qt::SizeHintRole )
				return model()->headerData ( section, orientation, Qt::UserRole + role );
				...
				case Qt::Horizontal :
				{
					...
					if ( section < m_BandList.size() )
					{
			 			...
			 			for ( int iSect = m_BandList.at ( section ).first; iSect < m_BandList.at ( section ).second; iSect++ )
			 				iWidth += qvariant_cast<QSize>( model()->headerData ( iSect, orientation, Qt::SizeHintRole ) ).width();
					}
		 								  
					QFontMetrics FM ( qvariant_cast<QFont>(model()->headerData ( section, orientation, Qt::UserRole + Qt::FontRole )) );
...
class QExtendedGridViewPrivate
{
	...
	public :
		...
		QHeaderView * CreateHeader ( const QString & Name, Qt::Orientation orientation, int iRowHeight )
		{
		  ...
		  Res->setResizeMode ( QHeaderView::Fixed );
...
// *****************************************************************************
void QExtendedGridView::focusInEvent ( QFocusEvent * ) { focusNextChild(); }
...
//    ScrollBar -->    <|_______________________________|>
  ...
  d->m_GridWidgetLock->setMidLineWidth ( 1 );
	...
	d->m_GridWidgetLock->header()->setMovable ( false );
  ...
  d->m_GridWidget->setMidLineWidth ( 1 );
	...
	d->m_GridWidget->header()->setMovable ( false );
...
void QExtendedGridView::setModel ( QAbstractItemModel * model )
{
	...
	d->m_GridTopBandModel->setModel ( model );
	d->m_GridFooterModel->setModel ( model );
